...
class Camera:
    ...
    def setAOI(self):
        self.ret = ueye.is_AOI(self.hcam, ueye.IS_AOI_IMAGE_GET_AOI, self.rectAOI, ueye.sizeof(self.rectAOI))
        if self.ret != ueye.IS_SUCCESS:
            print("is_AOI ERROR")
    ...
    def setmemory(self):
        self.ret = ueye.is_AllocImageMem(self.hcam, self.width, self.height, self.nBitsPerPixel, self.pcImageMemory, self.MemID)
        if self.ret != ueye.IS_SUCCESS:
            print("is_AllocImageMem ERROR")
        else:
            # Makes the specified image memory the active memory
            self.ret = ueye.is_SetImageMem(self.hcam, self.pcImageMemory, self.MemID)
            if self.ret != ueye.IS_SUCCESS:
    ...
    def acquireimage(self):
        ...
        if self.ret != ueye.IS_SUCCESS:
            print("is Error")
        self.ret = ueye.is_InquireImageMem(self.hcam, self.pcImageMemory, self.MemID, self.width, self.height, self.nBitsPerPixel, self.pitch)
        if self.ret != ueye.IS_SUCCESS:
